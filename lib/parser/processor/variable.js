...
utils = utils.extend({
    ...
    name: function(state) {
        ...
        state.item.internalName = state.lexicalEnvironment().record().getUniqueName(state.item.name);
        state.next();
    },
...
module.exports = {
    ...
    tokenType: "keyword/"+constants.variableKeyword,
    ...
    process: function(state) {
       // if(state.scope === "value") return state.error("Unexpected variable statement.");
        ...
        if(!state.hasScope('on')) state.item.global = true;
